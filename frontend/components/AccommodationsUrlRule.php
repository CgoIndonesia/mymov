...
class AccommodationsUrlRule extends Object implements UrlRuleInterface
{
    ...
    public function init()
    {
        ...
        $posts = Accommodations::find()->indexBy('id')->all();
    ...
    public function parseRequest($manager, $request)
    {
        ...
        if (isset ($path[1]) AND $path[0] == 'accommodations') {
            $alias = $path[1];
            if (isset($this->indexedByAlias[$alias])) {
                return ['accommodations/view', ['id' => $this->indexedByAlias[$alias]]];
            }
            return false;
        }
        return false;  // this rule does not apply
